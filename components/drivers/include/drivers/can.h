 ...
 *在第46行 ，item中增加了BLANK选项，用于增加标识符组，其中在RT_CAN_FILTER_ITEM_INIT(0x1314,ide,rtr,mode,mask,blank,ind,args)中blank的数字为0即为第0组的过滤器增加id(0x1314),
 *在RT_CAN_FILTER_ITEM_INIT(0x520,ide,rtr,mode,mask,blank,ind,args)中blank的数字为1即为第1组的过滤器增加id(0x520),
 *在RT_CAN_FILTER_ITEM_INIT(0x2020,ide,rtr,mode,mask,blank,ind,args)中blank的数字为2即为第2组的过滤器增加id(0x2020),
 *
 */
...
#ifdef RT_CAN_USING_HDR
#define RT_CAN_FILTER_ITEM_INIT(id,ide,rtr,mode,mask,blank,ind,args) \
     {(id), (ide), (rtr), (mode), (mask), -1,(blank), (ind), (args)}
#define RT_CAN_FILTER_STD_INIT(id,blank,ind,args) \
     RT_CAN_FILTER_ITEM_INIT(id,0,0,0,0xFFFFFFFF,blank,ind,args)
#define RT_CAN_FILTER_EXT_INIT(id,blank,ind,args) \
     RT_CAN_FILTER_ITEM_INIT(id,1,0,0,0xFFFFFFFF,blank,ind,args)
#define RT_CAN_STD_RMT_FILTER_INIT(id,blank,ind,args) \
     RT_CAN_FILTER_ITEM_INIT(id,0,1,0,0xFFFFFFFF,blank,ind,args)
#define RT_CAN_EXT_RMT_FILTER_INIT(id,blank,ind,args) \
     RT_CAN_FILTER_ITEM_INIT(id,1,1,0,0xFFFFFFFF,blank,ind,args)
#define RT_CAN_STD_RMT_DATA_FILTER_INIT(id,blank,ind,args) \
     RT_CAN_FILTER_ITEM_INIT(id,0,0,1,0xFFFFFFFF,blank,ind,args)
#define RT_CAN_EXT_RMT_DATA_FILTER_INIT(id,blank,ind,args) \
     RT_CAN_FILTER_ITEM_INIT(id,1,0,1,0xFFFFFFFF,blank,ind,args)
#else

#define RT_CAN_FILTER_ITEM_INIT(id,ide,rtr,mode,mask,blank) \
     {(id), (ide), (rtr), (mode), (mask), -1, (blank)}
#define RT_CAN_FILTER_STD_INIT(id,blank) \
     RT_CAN_FILTER_ITEM_INIT(id,0,0,0,0xFFFFFFFF,blank)
